  ...
  if (req.url === '/api/send-email' && req.method === 'POST') {
    ...
    req.on('end', async () => {
      try {
        ...
        if (!adminEmail) {
          console.warn('⚠️ PETCARE_ADMIN_EMAIL이 설정되지 않았습니다. 기본값으로 설정됩니다.');
          res.writeHead(500, { 'Content-Type': 'application/json' });
          res.end(JSON.stringify({
            error: 'Admin email configuration error',
            details: '환경변수 PETCARE_ADMIN_EMAIL을 .env.local에 설정해주세요.\n예시: PETCARE_ADMIN_EMAIL=your_email@gmail.com'
          }));
          return;
        }
...
server.listen(API_PORT, () => {
  ...
  console.log(`🏢 회사정보: ${COMPANY_INFO.name} (${COMPANY_INFO.phone})`);
});
