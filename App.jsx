  ...
  const handleConsultationSubmit = async (e) => {
    ...
    try {
      const response = await fetch('/api/send-email', {
        ...
        body: JSON.stringify({
          to: import.meta.env.VITE_PETCARE_ADMIN_EMAIL || 'hejunl@hanmail.net',
          replyTo: formData.email,
          subject: `[PetCare+] 상담신청: ${formData.name}님 (${formData.petName})`,
          ...formData
        })
      });
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {[
              ...
              { icon: '💬', title: 'AI 24시간 상담', desc: '챗봇과 언제든 펫보험 질문 상담받기', action: () => setShowChatBot(true) }
            ].map((item, idx) => (
              <button key={idx} onClick={item.action} className="bg-gradient-to-br from-gray-50 to-gray-100 hover:shadow-lg p-6 rounded-xl transition text-left">
                ...
                <span className="inline-block bg-blue-500 text-white px-4 py-2 rounded-lg font-semibold">시작하기 →</span>
              </button>
            ))}
          </div>
        </div>
